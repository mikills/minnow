...
func NewMongoMediaStore(ctx context.Context, coll *mongo.Collection) (*MongoMediaStore, error) {
	...
	_, err := s.Collection.Indexes().CreateMany(ctx, []mongo.IndexModel{
		{
			Keys:    bson.D{{Key: mongoKeyKBID, Value: 1}, {Key: "state", Value: 1}},
			Options: options.Index().SetName("kb_state"),
		},
		{
			Keys:    bson.D{{Key: mongoKeyKBID, Value: 1}, {Key: "filename", Value: 1}},
			Options: options.Index().SetName("kb_filename"),
		},
		{
			Keys: bson.D{{Key: mongoKeyKBID, Value: 1}, {Key: "idempotency_key", Value: 1}},
			Options: options.Index().
				SetName("kb_idempotency").
				SetUnique(true).
				SetPartialFilterExpression(bson.D{{Key: "idempotency_key", Value: bson.D{{Key: "$gt", Value: ""}}}}),
		},
	})
	if err != nil {
		return nil, err
...
func (s *MongoMediaStore) Put(ctx context.Context, m MediaObject) error {
	...
	_, err := s.Collection.ReplaceOne(ctx, bson.M{mongoKeyID: m.ID}, toMongoMedia(m),
...
func (s *MongoMediaStore) Get(ctx context.Context, mediaID string) (*MediaObject, error) {
	...
	if err := s.Collection.FindOne(ctx, bson.M{mongoKeyID: mediaID}).Decode(&doc); err != nil {
...
func (s *MongoMediaStore) FindByIdempotency(ctx context.Context, kbID, idempotencyKey string) (*MediaObject, error) {
	...
	var doc mongoMediaDoc
	err := s.Collection.FindOne(ctx, bson.M{mongoKeyKBID: kbID, "idempotency_key": idempotencyKey}).Decode(&doc)
	if err != nil {
		if errors.Is(err, mongo.ErrNoDocuments) {
			return nil, nil
		}
...
func (s *MongoMediaStore) List(ctx context.Context, kbID, prefix string, after string, limit int) (MediaPage, error) {
	filter := bson.M{mongoKeyKBID: kbID}
	if prefix != "" {
...
func (s *MongoMediaStore) UpdateState(
	ctx context.Context,
	mediaID string,
	state MediaState,
	...
	res, err := s.Collection.UpdateOne(ctx, bson.M{mongoKeyID: mediaID}, update)
	if err != nil {
		return err
	}
...
func (s *MongoMediaStore) ListByState(
	ctx context.Context,
	kbID string,
	state MediaState,
	after string,
...
func (s *MongoMediaStore) findPage(ctx context.Context, filter bson.M, limit int, pageToken string) (MediaPage, error) {
	...
	opts := options.Find().SetSort(bson.D{{Key: mongoKeyID, Value: 1}}).SetLimit(int64(limit))
...
func (s *MongoMediaStore) Delete(ctx context.Context, mediaID string) error {
	_, err := s.Collection.DeleteOne(ctx, bson.M{mongoKeyID: mediaID})
	return err
}
