...
func newTestMongoStore(t *testing.T) (*MongoStore, *mongo.Client) {
	t.Helper()
	coll, client := newTestMongoCollection(t, "events")
	s, err := NewMongoStore(context.Background(), coll, client)
	require.NoError(t, err)
	return s, client
}

func startConcurrentClaim(
	ctx context.Context,
	s *MongoStore,
	wg *sync.WaitGroup,
	winners, noneAvailable *atomic.Int32,
) {
	wg.Add(1)
	go func() {
		defer wg.Done()
