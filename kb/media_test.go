...
// newMediaHarness returns a kb.TestHarness wired with an in-memory kb.MediaStore
// and registered cleanup - the shared setup across every media test.
func newMediaHarness(t *testing.T, kbID string) (*kb.TestHarness, kb.MediaStore) {
	t.Helper()
	h := kb.NewTestHarness(t, kbID).Setup()
	t.Cleanup(h.Cleanup)
	store := kb.NewInMemoryMediaStore()
...
func TestSanitizeMediaFilename(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.in, func(t *testing.T) {
			got, err := kb.SanitizeMediaFilename(tc.in)
			if tc.wantErr {
				require.Error(t, err)
				return
			}
...
func TestMediaBlobKey(t *testing.T) {
	require.Equal(t, "kbs/my-kb/media/med-1/doc.pdf", kb.MediaBlobKey("my-kb", "med-1", "doc.pdf"))
}

func TestUploadMedia(t *testing.T) {
	t.Run("rejects empty body", func(t *testing.T) {
		h, _ := newMediaHarness(t, "media-empty")
		_, err := h.KB().UploadMedia(context.Background(), kb.MediaUploadInput{
			KBID:     "kb1",
			Filename: "empty.bin",
	...
	t.Run("happy path stores pending media with metadata", func(t *testing.T) {
		...
		res, err := h.KB().UploadMedia(context.Background(), kb.MediaUploadInput{
			KBID:        "kb1",
			Filename:    "hello.txt",
		...
		require.NoError(t, err)
		require.Equal(t, kb.MediaStatePending, got.State)
	...
	t.Run("idempotency key returns the existing media", func(t *testing.T) {
		...
		first, err := h.KB().UploadMedia(context.Background(), kb.MediaUploadInput{
			KBID:           "kb1",
			Filename:       "hello.txt",
			ContentType:    "text/plain",
			Body:           strings.NewReader("hello world"),
		...
		second, err := h.KB().UploadMedia(context.Background(), kb.MediaUploadInput{
			KBID:           "kb1",
...
func TestMediaLifecycle(t *testing.T) {
	t.Run("PromoteReferencedMedia transitions pending media to active idempotently", func(t *testing.T) {
		...
		require.Equal(t, kb.MediaStateActive, got.State)
	...
	t.Run("ValidateDocumentReferences rejects unknown media_id", func(t *testing.T) {
		...
		err := h.KB().
			ValidateDocumentReferences(context.Background(), "kb1", []kb.Document{{ID: "d", Text: "x", MediaIDs: []string{"missing"}}})
	...
	t.Run("ValidateDocumentReferences rejects caller-supplied blob_key", func(t *testing.T) {
		...
		err := h.KB().
			ValidateDocumentReferences(context.Background(), "kb1", []kb.Document{{ID: "d", Text: "x", MediaRefs: []kb.ChunkMediaRef{{MediaID: "m1", BlobKey: "evil"}}}})
...
func TestMediaGC(t *testing.T) {
	t.Run("mark tombstones unreferenced pending media past TTL", func(t *testing.T) {
		...
		got, _ := store.Get(context.Background(), res.MediaID)
		require.Equal(t, kb.MediaStateTombstoned, got.State)
	})
	...
	t.Run("sweep deletes tombstoned media past the grace window", func(t *testing.T) {
		...
		require.NoError(t, store.UpdateState(context.Background(), res.MediaID,
			kb.MediaStateTombstoned, time.Now().Add(-2*time.Hour).UnixMilli()))
		...
		require.ErrorIs(t, err, kb.ErrMediaNotFound)
	})

	t.Run("scheduler registers mark and sweep jobs", func(t *testing.T) {
		h, _ := newMediaHarness(t, "media-gc-scheduler")

		s := kb.NewScheduler(kb.NewInMemoryWriteLeaseManager(), time.Minute, nil, nil)
		require.NoError(t, h.KB().RegisterDefaultJobs(s))
		require.Contains(t, s.JobIDs(), kb.MediaGCMarkJobID)
		require.Contains(t, s.JobIDs(), kb.MediaGCSweepJobID)
	})
}
