...
func newTestMongoMediaStore(t *testing.T) *kb.MongoMediaStore {
	t.Helper()
	coll, _ := newTestMongoCollection(t, "media")
	s, err := kb.NewMongoMediaStore(context.Background(), coll)
	require.NoError(t, err)
	return s
}

func mkMedia(id, kbID string) kb.MediaObject {
	return kb.MediaObject{
...
func TestMongoMediaStore(t *testing.T) {
	...
	t.Run("update state and list by state", func(t *testing.T) {
		...
		require.NoError(t, s.UpdateState(ctx, "m1", kb.MediaStateTombstoned, time.Now().UnixMilli()))

		tomb, err := s.ListByState(ctx, "", kb.MediaStateTombstoned, "", 0)
		require.NoError(t, err)
		require.Len(t, tomb.Items, 1)
		require.Equal(t, "m1", tomb.Items[0].ID)

		pending, err := s.ListByState(ctx, "kb", kb.MediaStatePending, "", 0)
		require.NoError(t, err)
		require.Len(t, pending.Items, 1)
		require.Equal(t, "m2", pending.Items[0].ID)

		err = s.UpdateState(ctx, "no-such-id", kb.MediaStateActive, 0)
		require.ErrorIs(t, err, kb.ErrMediaNotFound)
	})
