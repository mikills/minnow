...
func (s *Scheduler) runLeasedJobTrampoline(rootCtx context.Context, id string, fn SchedulerJob, leaseTTL time.Duration) {
	defer s.jobWG.Done()
...
func (s *Scheduler) handleJobTrampolinePanic(id string, recovered any) {
	...
	s.logger.Error("scheduler job trampoline panic", "job_id", id, "panic", fmt.Sprint(recovered))
	s.report(id, SchedulerOutcomeFailed, 0, fmt.Errorf("scheduler job trampoline panicked: %v", recovered))
}
...
func (s *Scheduler) runWithLease(ctx context.Context, id string, fn SchedulerJob, leaseTTL time.Duration) (SchedulerOutcome, error) {
	...
	defer func() {
		if err := s.leaseMgr.Release(context.WithoutCancel(ctx), lease); err != nil {
			s.logger.Warn("scheduler lease release failed", "job_id", id, "error", err)
		}
	}()
...
func (s *Scheduler) notifyObserverSafely(id string, outcome SchedulerOutcome, dur time.Duration, err error) {
	defer func() {
		if r := recover(); r != nil {
			slog.Default().Warn("scheduler observer panicked", "job_id", id, "panic", fmt.Sprint(r))
		}
	}()
